...
/// 图表当前位置-被动调用, 由图表滚动事件触发.
///
/// - left: 处于最左端.
/// - toLeft: 正在向左滚动.
/// - toRight: 正在向右滚动.
/// - right: 处于最右端.
@objc public enum CHChartViewEdgePosition: Int {
    case left, toLeft, toRight, right
}
...
@objc public protocol CHKLineChartDelegate: class {
    ...
    /// 图表滚动到的位置.
    ///
    @objc optional func kLineChart(chart: CHKLineChartView, positionDidChanged toPosition: CHChartViewEdgePosition)
}
...
open class CHKLineChartView: UIView {
    ...
    var theBorderColor: UIColor = UIColor.gray
...
//        context?.fill (CGRect (x: 0, y: 0, width: self.bounds.size.width,height: self.bounds.size.height))
    ...
    fileprivate func drawSection(_ section: CHSection) {
        ...
        if self.theBorderWidth.bottom > 0 {
            ...
            borderPath.append(UIBezierPath(rect: CGRect(x: section.frame.origin.x + section.padding.left, y: section.frame.size.height + section.frame.origin.y, width: section.frame.size.width - section.padding.left, height: self.theBorderWidth.bottom)))
        ...
        if self.theBorderWidth.top > 0 {
            ...
            borderPath.append(UIBezierPath(rect: CGRect(x: section.frame.origin.x + section.padding.left, y: section.frame.origin.y, width: section.frame.size.width - section.padding.left, height: self.theBorderWidth.top)))
        ...
        if self.theBorderWidth.left > 0 {
            ...
            borderPath.append(UIBezierPath(rect: CGRect(x: section.frame.origin.x + section.padding.left, y: section.frame.origin.y, width: self.theBorderWidth.left, height: section.frame.size.height)))
        ...
        if self.theBorderWidth.right > 0 {
            ...
            borderPath.append(UIBezierPath(rect: CGRect(x: section.frame.origin.x + section.frame.size.width - section.padding.right, y: section.frame.origin.y, width: self.theBorderWidth.left, height: section.frame.size.height)))
            ...
            if direction {
                ...
                if self.plotCount > (self.rangeTo-self.rangeFrom) {
                    if self.rangeFrom - interval >= 0 {
                        ...
                        self.delegate?.kLineChart?(chart: self, positionDidChanged: .toLeft)
                    } else {
                        self.rangeFrom = 0
                        self.rangeTo -= self.rangeFrom
                        self.delegate?.kLineChart?(chart: self, positionDidChanged: .left)
                    }
                    self.drawLayerView()
                }
            } else {
                ...
                if self.plotCount > (self.rangeTo-self.rangeFrom) {
                    if self.rangeTo + interval <= self.plotCount {
                        self.rangeFrom += interval
                        self.rangeTo += interval
                        self.delegate?.kLineChart?(chart: self, positionDidChanged: .toRight)
                    } else {
                        self.rangeFrom += self.plotCount - self.rangeTo
                        self.rangeTo  = self.plotCount
                        self.delegate?.kLineChart?(chart: self, positionDidChanged: .right)
