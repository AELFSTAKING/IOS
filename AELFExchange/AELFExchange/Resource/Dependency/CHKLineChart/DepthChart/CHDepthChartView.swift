...
open class CHDepthChartView: UIView {
    ...
    /// MARK: - 成员变量
    open var bidColor: (stroke: UIColor, fill: UIColor, lineWidth: CGFloat, gradientColorEnd: UIColor) = (.green, .green, 1, .green)
    open var askColor: (stroke: UIColor, fill: UIColor, lineWidth: CGFloat, gradientColorEnd: UIColor) = (.red, .red, 1, .red)
    ...
    /// 显示边线上左下有
    open var theBorderWidth: (top: CGFloat, left: CGFloat, bottom: CGFloat, right: CGFloat) = (0.25, 0.25, 0.25, 0.25)
    ...
    open var style: CHKLineChartStyle! {           //显示样式
        didSet {
            ...
            self.theBorderWidth = self.style.borderWidth
    ...
    func setSelectedIndexByPoint(_ point: CGPoint) {
        ...
        if self.bidChartOnDirection == .left{
            if index <= self.bidItems.count{
                selectedItem = self.bidItems[self.bidItems.count-index-1] //todo: 边界值存在越界问题.
            }else{
...
extension CHDepthChartView {
    ...
    func drawLayerView() {
        ...
        //绘制图表框架
        //self.drawChartFrame() // 暂时关闭不需要的绘制.
    ...
    fileprivate func drawChartFrame() {
        ...
        //画低部边线
        if self.theBorderWidth.bottom > 0 {
            
            borderPath.append(UIBezierPath(rect: CGRect(x: self.bounds.origin.x + self.padding.left, y: self.bounds.size.height + self.bounds.origin.y, width: self.bounds.size.width - self.padding.left, height: self.theBorderWidth.bottom)))
            
        }
        
        //画顶部边线
        if self.theBorderWidth.top > 0 {
            
            borderPath.append(UIBezierPath(rect: CGRect(x: self.bounds.origin.x + self.padding.left, y: self.bounds.origin.y, width: self.bounds.size.width - self.padding.left, height: self.theBorderWidth.top)))
            
        }
        
        
        //画左边线
        if self.theBorderWidth.left > 0 {
            
            borderPath.append(UIBezierPath(rect: CGRect(x: self.bounds.origin.x + self.padding.left, y: self.bounds.origin.y, width: self.theBorderWidth.left, height: self.bounds.size.height)))
            
        }
        
        
        //画右边线
        if self.theBorderWidth.right > 0 {
            
            borderPath.append(UIBezierPath(rect: CGRect(x: self.bounds.origin.x + self.bounds.size.width - self.padding.right, y: self.bounds.origin.y, width: self.theBorderWidth.left, height: self.bounds.size.height)))
    ...
    func drawChart() -> [(CGRect, String)]{
        ...
        if self.bidChartOnDirection == .right{
            ...
            let bidsRect = CGRect(x: buy1X, y: bidsY, width: 60, height: 18)
            let buy1Rect = CGRect(x: bidsX, y: buy1Y, width: 60, height: 18)
    ...
    func drawDepthChart(items: [CHKDepthChartItem],
        ...
        // 【二】绘制填充区域
        let gEndColor = startX <= 0 ?  self.bidColor.gradientColorEnd: self.askColor.gradientColorEnd // 暂时,根据绘制开始位置确定渐变色末尾颜色.
        let gradientColor = UIColor(
            gradientStyle: .topToBottom,
            withFrame: CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width/2.0, height: self.frame.height),
            andColors: [gEndColor ,fillColor]
        )
