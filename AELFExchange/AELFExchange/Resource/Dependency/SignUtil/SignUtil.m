...
+ (NSString *)rsa_decryptString:(NSString *)str privateKeyWithContentsOfFile:(NSString *)path password:(NSString *)password {
    if (!str || !path) return nil;
 ...
 */
static SecKeyRef __rsaKey;
+ (NSData *)rsaSHA256SignData:(NSData *)plainData privateKey:(NSString *)privateKey {
    if (!__rsaKey) {
        __rsaKey = [SignUtil addPrivateKey:privateKey];
    }
    SecKeyRef key = __rsaKey;
 ...
 */
static SecKeyRef __rsaPublicKey;
+ (BOOL)rsaSHA256VerifyData:(NSData *)plainData withSignature:(NSData *)signature publicKey:(NSString *)publicKey {
    if (!__rsaPublicKey) {
        __rsaPublicKey = [SignUtil addPublicKey:publicKey];
    }
    
    SecKeyRef key = __rsaPublicKey;
